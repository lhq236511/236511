...
Game.prestigeData = (function(){
	...
	**************/
	...
	instance.empowerManualGains = {
		...
		onApply: function(){
			...
			// new
			// 为了(在 Game.resources 中的 var id 条目){
	...
	instance.startingStorage = {
		...
		onApply: function(){
			// old
			var newStorage = 10000;
			for(var i = 0; i < resources.length; i++){
				if(window[resources[i] + "Storage"] <= 10000){
					window[resources[i] + "Storage"] = newStorage;
					window[resources[i] + "NextStorage"] = newStorage * 20;
				}
			}
		...
		remove: function(){
	    	for(var i = 0; i < resources.length; i++){
				if(window[resources[i] + "Storage"] <= 20000){
					window[resources[i] + "Storage"] = 20000;
					window[resources[i] + "NextStorage"] = 20000 * 20;
				} else {
					window[resources[i] + "Storage"] /= 10000;
					window[resources[i] + "NextStorage"] /= 10000;
				}
			}
	    },
		achieved: false
	...
	**************/
	...
	instance.energyEff = {
		...
		opinion: 100,
		onApply: function(){
			Game.tech.entries["energyEfficiencyResearch"].maxLevel += 100;
		},
		remove: function(){
	    	Game.tech.entries["energyEfficiencyResearch"].maxLevel = 100;
	    },
	...
	*************/
	...
	// 例子.回收利用 = {
	// 	姓名: "回收利用",
	// 	Desc: “与其销毁机器，不如将它们回收利用，收取50% 的费用！",
	// 	成本: 29,
	// 	类别: 「风信子」,
	// 	实现: 错误
	// };
