var Game = (function() {
    ...
    instance.fastUpdate = function(self, delta) {
        ...
        if(delta > 1) {
            console.log("你已经离开了 " + Game.utils.getTimeDisplay(delta));
        }
    };
    ...
    instance.import = function() {
        ...
        if (!text.trim()) return console.warn("没有提供导入保存.");
        if(text.length % 4 !== 0) {
            console.log("字符串未被有效的 base64编码: " + text.length + ' (' + text.length % 4 + ')');
            return;
        }
    ...
    instance.loadDelay = function (self, delta) {
        ...
        // 在 setInterval 中执行此操作，这样即使在窗口处于非活动状态时也可以调用它
